...
function ReviewedUsers() {
  ...
  const ApiHandler = async () => {
    const result = await fetch(
      "http://localhost:8000/api/getUserReviewsbyUser/" + userid
    );
  ...
  function DeleteReview(key) {
    fetch(
      "http://localhost:8000/api/deleteUserReview/" + key + "?_method=DELETE",
      {
        method: "DELETE",
      }
    ).then((result) => {
  ...
  return (
    <div className="flex-grow-1 ms-3">
      <h3 className="profiletitle2">Posted Reviews on Users</h3>
      <MDBTable hover style={{ textAlign: "center" }}>
        ...
        {reviewData.map((item) => (
          <MDBTableBody key={reviewData.id}>
            <tr>
              <td>
              ...
              <td>
                <p className="fw-normal mb-2">{item.rating}</p>
              </td>
              <td>
                <MDBBtn
                  size="sm"
